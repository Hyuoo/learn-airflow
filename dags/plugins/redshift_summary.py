...
def build_summary_table(dag_root_path, dag, tables_load, redshift_conn_id, start_task=None):
    """
    DAG에서 dag_root_path 경로를 입력받아,
    ${dag_root_path}/config/의 *.py에 딕셔너리 내용들을 리스트 형태로 반환
    
    각 리스트 요소는 하나의 설정내용
    -> 파일 하나 당 RedshiftSummaryOperator(task) 하나 생성
    """
    ...
    for table_name in tables_load:
        # /config/*.py를 모두 읽지만, 파일의 key["table"] 내용이 tables_load에 포함 된 테이블만 실행함.
